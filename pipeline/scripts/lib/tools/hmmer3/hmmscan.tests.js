...
describe('HmmscanReaderStream', function() {
	let inStream = fs.createReadStream(path.resolve(__dirname, 'demo_hmmscanResults.txt')),
		hmmscanStream = new hmmscanReaderStream(),
		results = []
	it('performing hmmscan parser stream', function(done) {
		inStream.pipe(hmmscanStream)
			.on('data', (result) => {
				let HmmscanResultObject = new HmmscanResult(result)
				results.push(HmmscanResultObject)
			})
			.on('finish', () => {
